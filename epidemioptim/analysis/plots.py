...
def setup_figure(
    xlabel=COSTS_LABELS[0],
    ylabel=COSTS_LABELS[1],
    xlim=XLIM,
    ylim=YLIM,
    figsize=(18, 18),
):
...
def save_fig(path, artists):
    plt.savefig(
        os.path.join(path), bbox_extra_artists=artists, bbox_inches="tight", dpi=DPI
    )
    plt.close("all")
...
def pareto_plot(central, error):
    ...
    if PLOT_STD:
        ...
        for i in range(n_points):
            ellipses.append(
                Ellipse(xy=central[i], width=error[i][0], height=error[i][1], alpha=0.1)
            )
        for e in ellipses:
            ax.add_artist(e)
...
def extract_res(folder, algo, max_costs, min_costs):
    ...
    for fold in list_folds:
        if "res" not in fold:
            ...
            std = res[
                "F_std"
            ]  # std of the points in Pareto front (over n evaluation episodes)
            ...
            if algo == "DQN":
                goals.append(
                    res["G_all"].flatten()
                )  # collect the goal for DQN policies, so as to pull them into populations of policies
                mean = mean.flatten()
                std = std.flatten()
    ...
    if algo == "DQN":
        ...
        for i_run in range(nb_runs):
            inds = np.array(
                [
                    np.argwhere(np.array(goals).flatten() == unique_goals[i])[i_run]
                    for i in range(unique_goals.size)
                ]
            ).flatten()
    ...
    np.savetxt(
        folder + "res/all_data_pareto_std_{}.txt".format(SWITCH), np.array(all_data_std)
    )
...
def beta_plot(folder):
            ...
            if "G_all" in data.keys():
                for g, f, f_std in zip(data["G_all"], data["F_all"], data["F_std_all"]):
                    ...
                    if str(g) not in all_data.keys():
                        all_data[str(g)] = dict(F_all=[], F_std_all=[])
                        goals.append(g)
    ...
    if len(goals) > 0:
        ...
        data = np.swapaxes(
            np.array([all_data[k]["F_all"] / np.array([XLIM[1], 110]) for k in keys]),
            axis1=0,
            axis2=1,
        )
        central = LINE(data)
        ...
        artists, ax = setup_figure(
            r"$\beta$", "costs (% max)", xlim=[0, 1], ylim=[0, 1], figsize=(15, 10)
        )
        ...
        for i_c in range(nb_costs):
            ...
            plt.fill_between(
                beta_y, errors[0][:, i_c], errors[1][:, i_c], color=c, alpha=ALPHA
            )
        leg = plt.legend(
            COST_LABELS2,
            loc="upper center",
...
def plot_multi_algo(folder):
    ...
    for cond in list_conds:
        if "res" not in cond:
            ...
            data.append(
                np.loadtxt(
                    RES_FOLDER + cond + "/res/all_data_pareto_{}.txt".format(SWITCH)
                )
            )
            data_std.append(
                np.loadtxt(
                    RES_FOLDER + cond + "/res/all_data_pareto_std_{}.txt".format(SWITCH)
                )
            )
    ...
    for i in range(n_conds):
        ...
        plus_or_minus = np.array(
            [areas[i].mean() > areas[j].mean() for j in range(n_conds)]
        )
        sig = np.array([p_vals[i, j] < 0.05 for j in range(n_conds)])
        msg = "{}: {:.2f} +/- {:.2f}.".format(
            labels[i], areas[i].mean(), areas[i].std()
        )
    ...
    if SWITCH:
        ...
        artists, ax = setup_figure(
            figsize=(15, 10),
            ylabel=r"# Deaths $(\times 10^3)$",
            ylim=(0, YLIM[1] * Y_SCALE * 1.4),
        )
    else:
        X_SCALE = 1 / 1000
        Y_SCALE = 1
        ...
        artists, ax = setup_figure(
            figsize=(15, 10),
            xlabel=r"# Deaths $(\times 10^3)$",
            xlim=(0, XLIM[1] * X_SCALE),
            ylim=(0, YLIM[1] * 1.35),
        )
    data *= Y_SCALE
    central = LINE(data)
    ...
    for i_c in range(central.shape[0]):
        ...
        plt.fill_between(
            inds, errs[0][i_c, :], errs[1][i_c, :], color=colors[i_c], alpha=ALPHA
        )
        if i_c in p_vals.keys():
            counter += 1
            inds_inf = np.argwhere(
                np.logical_and(
                    p_vals[i_c] < 0.05,
                    central[i_c, inds_freq] < central[i_ref, inds_freq],
                )
            ).flatten()
            inds_sup = np.argwhere(
                np.logical_and(
                    p_vals[i_c] < 0.05,
                    central[i_c, inds_freq] > central[i_ref, inds_freq],
                )
            ...
            if inds_inf.size > 0:
                plt.scatter(
                    inds[inds_freq][inds_inf],
                    np.ones([inds_inf.size]) * YLIM[1] * 0.92 * Y_SCALE
                    + delta_sig * counter,
                    color=colors[i_c],
                    s=180,
                    marker="o",
            ...
            if inds_sup.size > 0:
                plt.scatter(
                    inds[inds_freq][inds_sup],
                    np.ones([inds_sup.size]) * YLIM[1] * 0.92 * Y_SCALE
                    + delta_sig * counter,
                    color=colors[i_c],
                    s=250,
                    marker="*",
