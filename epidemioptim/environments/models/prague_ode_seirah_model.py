...
PATH_TO_FITTED_PARAMS = (
    get_repo_path() + "/data/model_data/estimatedIndividualParameters.csv"
)
...
class PragueOdeSeirahModel(BaseModel):
    def __init__(
        self, region="IDF", stochastic=False, noise_params=0.1, range_delay=(0, 21)
    ):
        """
        ...
        self.pop_sizes = dict(
            zip(self.fitted_params["id"], (self.fitted_params["popsize"]))
        )
        assert region in self._regions, "region code should be one of " + str(
            self._regions
        )
        ...
        super().__init__(
            internal_states_labels=["S", "E", "I", "R", "A", "H"],
    ...
    def define_params_and_initial_state_distributions(self):
        ...
        label2ind = dict(
            zip(list(self.fitted_cov.columns), np.arange(len(self.fitted_cov.columns)))
        )
        for i in self.fitted_params.index:
            r = self.fitted_params["id"][i]  # region
            self._all_internal_params_distribs[r] = dict(
                b_fit=LogNormalDist(
                    params=mv2musig(
                        self.fitted_params["b1_mean"][i],
                        self.fitted_cov["b1_pop"][label2ind["b1_pop"]],
                    ),
                    stochastic=self.stochastic,
                ),
                r_fit=NormalDist(
                    params=[0.034, 0.034 * self.noise], stochastic=self.stochastic
                ),
                N=DiracDist(
                    params=self.fitted_params["popsize"][i], stochastic=self.stochastic
                ),
                N_av=DiracDist(
                    params=float(np.mean(self.fitted_params["popsize"])),
                    stochastic=self.stochastic,
                ),
                Dq_fit=LogNormalDist(
                    params=mv2musig(
                        self.fitted_params["Dq_mean"][i],
                        self.fitted_cov["Dq_pop"][label2ind["Dq_pop"]],
                    ),
                    stochastic=self.stochastic,
                ),
                De=NormalDist(
                    params=[5.1, 5.1 * self.noise], stochastic=self.stochastic
                ),
                Dh=NormalDist(params=[30, 30 * self.noise], stochastic=self.stochastic),
                Di=NormalDist(
                    params=[2.3, 2.3 * self.noise], stochastic=self.stochastic
                ),
                alpha=NormalDist(
                    params=[0.55, 0.55 * self.noise], stochastic=self.stochastic
                ),
                icu=DiracDist(
                    params=self.fitted_params["ICUcapacity"][i],
                    stochastic=self.stochastic,
                ),
                beta1=NormalDist(
                    params=[
                        self.fitted_params["betaw1_mean"][i],
                ...
                beta2=NormalDist(
                    params=[
                        self.fitted_params["betaw2_mean"][i],
                ...
                beta3=NormalDist(
                    params=[
                        self.fitted_params["betaw3_mean"][i],
                ...
                beta4=NormalDist(
                    params=[
                        self.fitted_params["betaw4_mean"][i],
                        np.sqrt(self.fitted_cov["betaw4_pop"][label2ind["betaw4_pop"]]),
                    ],
                    stochastic=self.stochastic,
                ),
            )
            self._all_initial_state_distribs[r] = dict(
                E0=LogNormalDist(
                    params=mv2musig(
                        self.fitted_params["initE_mean"][i],
                        self.fitted_cov["initE_pop"][label2ind["initE_pop"]],
                    ),
                    stochastic=self.stochastic,
                ),
                I0=DiracDist(
                    params=self.fitted_params["I0_kalman_mean"][i],
                    stochastic=self.stochastic,
                ),
                R0=DiracDist(params=0, stochastic=self.stochastic),
                A0=DiracDist(params=1, stochastic=self.stochastic),  # is updated below
                H0=DiracDist(
                    params=self.fitted_params["H0_kalman_mean"][i],
                    stochastic=self.stochastic,
                ),
            )
    ...
    def _sample_initial_state(self):
        ...
        for k in self._all_initial_state_distribs[self.region].keys():
            self.initial_state[k] = self._all_initial_state_distribs[self.region][
                k
            ].sample()

        # A0 is computed as a function of I0 and r_fit (see Prague et al., 2020)
        self.initial_state["A0"] = (
            self.initial_state["I0"]
            * (1 - self.current_internal_params["r_fit"])
            / self.current_internal_params["r_fit"]
        )
        for k in self._all_initial_state_distribs[self.region].keys():
        ...
        self.initial_state["S0"] = self.current_internal_params["N"] - np.sum(
            [
                self.initial_state["{}0".format(s)]
                for s in self.internal_states_labels[1:]
            ]
        )
    ...
    def _sample_model_params(self):
        ...
        for k in self._all_internal_params_distribs[self.region].keys():
            self.initial_internal_params[k] = self._all_internal_params_distribs[
                self.region
            ][k].sample()
...
if __name__ == "__main__":
    # Get model
    model = PragueOdeSeirahModel(region="IDF", stochastic=False)
    ...
    plot_stats(t=time, states=model_states.transpose(), labels=labels, show=True)
