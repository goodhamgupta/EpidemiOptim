from epidemioptim.environments.cost_functions.costs.base_cost_function import (
    BaseCostFunction,
)
...
class GdpRecess(BaseCostFunction):
    def __init__(
        ...
        id_cost
        """
        super().__init__(scale_factor=scale_factor, range_constraints=range_constraints)
        ...
        self._K0 = (
            7481400 * 1e6 * self._ratio_pop
        )  # initial capital (prorata region pop)
        self._Y0 = 2317000 * 1e6 * self._ratio_pop  # initial GDP
        self._A = self._Y0 / (
            self._K0 ** self._alpha * (self._lambda_0 * self._N0) ** (1 - self._alpha)
    ...
    def compute_current_infected(self, state, label_to_id):
        ...
        if state.ndim == 2:
            infected = (
                state[:, label_to_id["I"]]
                + state[:, label_to_id["H"]]
                + self.ratio_death_to_R * state[:, label_to_id["R"]]
            )
        else:
            infected = (
                state[label_to_id["I"]]
    ...
    def compute_cost(self, previous_state, state, label_to_id, action, others={}):
        ...
        normal_GDP = (
            self._A
            * self._K0 ** self._alpha
            * (self._lambda_0 * self._N0) ** (1 - self._alpha)
        )
        current_GDP = [
            self._A
            * (self._K0 ** self._alpha)
            * ((1 - self._u[lockdown[i]]) * self._lambda_0 * (self._N0 - infected[i]))
            ** (1 - self._alpha)
    ...
    def compute_cumulative_cost(
        ...
        cumulative_cost = state[
            :, label_to_id["cumulative_cost_{}".format(self.id_cost)]
        ]
